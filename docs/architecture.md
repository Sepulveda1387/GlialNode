...
The configuration layer now also supports named space presets so new memory spaces can start with a coherent policy posture before finer overrides are applied, and those presets can be inspected, exported, registered locally, and reapplied through the library and CLI before use.
