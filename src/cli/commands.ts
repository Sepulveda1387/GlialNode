...
export function usageText(): string {
  return [
    ...
    "  glialnode execution-context recommend --task <text> [--repo-id <id>] [--project-id <id>] [--workflow-id <id>] [--agent-id <id>] [--features a,b] [--available-skills a,b] [--available-tools a,b] [--available-first-reads a,b] [--records <path>] [--metrics-db <path>] [--max-recommendations <n>] [--json]",
...
function runReleaseCommand(action: string, parsed: ParsedArgs): CommandResult {
  ...
  const report = buildReleaseReadinessReport({
    ...
    demoGreen: parseFlagBooleanDefaultFalse(parsed.flags["demo-green"]),
...
async function runExecutionContextCommand(
  ...
  if (action === "recommend") {
    ...
    const recommendation = recommendExecutionContext({
      ...
      availableFirstReads: parseCommaSeparatedList(parsed.flags["available-first-reads"]),
    ...
    return {
      lines: [
        ...
        `fallbackToNormalDiscovery=${recommendation.fallbackToNormalDiscovery ? "yes" : "no"}`,
        `driftedRecommendationCount=${recommendation.availabilityDiff.driftedRecommendationCount}`,
